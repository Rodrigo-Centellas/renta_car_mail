...
public class DUser {
    ...
    public List<String[]> save(String apellido,
        ...
        String query = "INSERT INTO \"User\" (apellido, ci, domicilio, nombre, telefono) " +
                "VALUES (?, ?, ?, ?, ?) RETURNING id";
        try (Connection conn = connection.connect();
             PreparedStatement ps = conn.prepareStatement(query)) {
    ...
    public List<String[]> update(int id,
                                 ...
                                 String nombre,
                                 int telefono) throws SQLException {
        String query = "UPDATE \"User\" SET apellido = ?, ci = ?, domicilio = ?, nombre = ?, telefono = ? " +
                "WHERE id = ?";
        try (Connection conn = connection.connect();
             PreparedStatement ps = conn.prepareStatement(query)) {
            ps.setString(1, apellido);
