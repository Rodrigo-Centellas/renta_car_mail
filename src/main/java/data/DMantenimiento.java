...
public class DMantenimiento {
    ...
    public List<String[]> get(int id) throws SQLException {
        List<String[]> mantenimiento = new ArrayList<>();
        String query = "SELECT * FROM \"Mantenimiento\" WHERE id = ?";
        PreparedStatement ps = connection.connect().prepareStatement(query);
        ps.setInt(1, id);
        ResultSet rs = ps.executeQuery();
        if (rs.next()) {
            mantenimiento.add(new String[]{
    ...
    public List<String[]> save(String descripcion, String nombre) throws SQLException {
        String query = "INSERT INTO \"Mantenimiento\" (descripcion, nombre) VALUES (?, ?) RETURNING id";
        PreparedStatement ps = connection.connect().prepareStatement(query);
        ...
        if (rs.next()) {
            ...
            throw new SQLException("Error al insertar mantenimiento. No se pudo recuperar el ID del mantenimiento.");
        }
    }

    public List<String[]> update(int id, String descripcion, String nombre) throws SQLException {
        String query = "UPDATE \"Mantenimiento\" SET descripcion = ?, nombre = ? WHERE id = ?";
        PreparedStatement ps = connection.connect().prepareStatement(query);
        ps.setString(1, descripcion);
        ps.setString(2, nombre);
        ps.setInt(3, id);
        if (ps.executeUpdate() == 0) {
            System.err.println("Error al modificar el mantenimiento");
    ...
    public List<String[]> delete(int id) throws SQLException {
        //List<String[]> mantenimiento = list();
        String query = "DELETE FROM \"Mantenimiento\" WHERE id = ?";
        PreparedStatement ps = connection.connect().prepareStatement(query);
        ps.setInt(1, id);
        if (ps.executeUpdate() == 0) {
            System.err.println("Error al eliminar mantenimiento");
            throw new SQLException();
        }
        return list();
    }
